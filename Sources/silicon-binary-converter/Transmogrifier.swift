...
enum Transmogrifier {
    private static func readBinary(atPath path: String, binaryType: BinaryType) -> (Data, [Data], Data) {
    ...
    private static func replaceVersionMin(_ data: Data, _ offset: UInt32, platform: Platform, minos: UInt32, sdk: UInt32) -> Data {
        var command = build_version_command(cmd: UInt32(LC_BUILD_VERSION),
                                            cmdsize: UInt32(MemoryLayout<build_version_command>.stride),
                                            platform: UInt32(platform.machOValue),
    ...
    private static func updateBuildVersion(_ data: Data, _ offset: UInt32, platform: Platform, minos: UInt32, sdk: UInt32) -> Data {
        ...
        new_command.platform = UInt32(platform.machOValue)
    ...
    private static func computeLoadCommandsEditor(_ loadCommandsData: [Data], binaryType: BinaryType) -> ((Data, Platform, UInt32, UInt32) -> Data) {

        if binaryType == .dynamic {
            return updateDylibFile
        }

        var contains_LC_VERSION_MIN = false
        var contains_LC_BUILD_VERSION = false
        for lc in loadCommandsData {
            let loadCommand = UInt32(lc.loadCommand)
            if [LC_VERSION_MIN_IPHONEOS, LC_VERSION_MIN_TVOS, LC_VERSION_MIN_WATCHOS, LC_VERSION_MIN_MACOSX].contains(Int32(loadCommand)) {
                contains_LC_VERSION_MIN = true
            } else if loadCommand == LC_BUILD_VERSION {
                contains_LC_BUILD_VERSION = true
            }
        }

        if contains_LC_VERSION_MIN == contains_LC_BUILD_VERSION {
            if contains_LC_BUILD_VERSION == true {
                fatalError("Bad Mach-O Object file: Both LC_VERSION_MIN_* and LC_BUILD_VERSION are present.\nEither one of them should be present")
            } else {
                fatalError("Bad Mach-O Object file: does not contain LC_VERSION_MIN_* or LC_BUILD_VERSION.\nEither one of them should be present")
            }
        }

        if contains_LC_VERSION_MIN {
            ...
            return updateLegacyObjectFile
        } else {
            return updateObjectFile
        }
    }


    static func updateObjectFile(lc: Data, platform: Platform, minos: UInt32, sdk: UInt32) -> Data {
        let cmd = Int32(bitPattern: lc.loadCommand)
        switch cmd {
            case LC_BUILD_VERSION:
                return updateBuildVersion(lc, 0, platform: platform, minos: minos, sdk: sdk)
            default:
                return lc
        }
    }

    static func updateLegacyObjectFile(lc: Data, platform: Platform, minos: UInt32, sdk: UInt32) -> Data {
        // `offset` is kind of a magic number here, since we know that's the only meaningful change to binary size
        // having a dynamic `offset` requires two passes over the load commands and is left as an exercise to the reader
        ...
        switch  cmd {
            ...
            case LC_VERSION_MIN_IPHONEOS, LC_VERSION_MIN_TVOS, LC_VERSION_MIN_MACOSX, LC_VERSION_MIN_WATCHOS:
                return replaceVersionMin(lc, offset, platform: platform, minos: minos, sdk: sdk)
    ...
    static func updateDylibFile(lc: Data, platform: Platform, minos: UInt32, sdk: UInt32) -> Data {
        // `offset` is kind of a magic number here, since we know that's the only meaningful change to binary size
        // having a dynamic `offset` requires two passes over the load commands and is left as an exercise to the reader
        let offset = UInt32(abs(MemoryLayout<build_version_command>.stride - MemoryLayout<version_min_command>.stride))
        let cmd = Int32(bitPattern: lc.loadCommand)
    ...
    static func processBinary(from sourcePath: String, to destinationPath: String, platform: Platform, minos: UInt32, sdk: UInt32, binaryType: BinaryType) {
        let (headerData, loadCommandsData, programData) = readBinary(atPath: sourcePath, binaryType: binaryType)

        let editor = computeLoadCommandsEditor(loadCommandsData, binaryType: binaryType)

        let editedCommandsData = loadCommandsData
            .map { return editor($0, platform, minos, sdk) }
            .merge()
        ...
        // save back to disk
        try! reworkedData.write(to: URL(fileURLWithPath: destinationPath))
    }
}
